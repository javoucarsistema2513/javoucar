...
const Dashboard: React.FC<DashboardProps> = ({ vehicle, user, onLogout }) => {
  ...
  const [parkedLocation, setParkedLocation] = useState<ParkingLocation | null>(() => {
    try {
      ...
      console.error("Erro ao carregar vaga salva:", e);
      return null;
    }
  });
  ...
  const initAudio = async () => {
    ...
    try {
      ...
      console.error("Erro init audio:", e);
    }
  };
  ...
  const startAlarm = () => {
    ...
    const playTone = (freq: number, startTime: number, duration: number) => {
      try {
        ...
        console.warn("Erro ao tocar tom do alarme:", e);
      }
    };
  ...
  const handleMarkLocation = () => {
    ...
    alert("Localização do veículo salva com sucesso!");
  };

  const handleOpenMaps = () => {
    if (!parkedLocation) {
      alert("Marque a localização do veículo primeiro!");
      return;
    }
  ...
  const handleShareLocation = async () => {
    ...
    const shareText = `📍 Meu carro (${vehicle?.model || 'veículo'}) está estacionado aqui: ${mapsUrl}`;

    try {
      if (navigator.share) {
        await navigator.share({
          title: 'Localização do meu Veículo',
          text: shareText,
          url: mapsUrl
        });
      } else {
  ...
  const handleSendAlert = async (messageText: string) => {
    ...
    if (!plateToSearch || plateToSearch.length < 7) { 
      setErrorMsg('Placa inválida (mín. 7 dígitos)'); 
      return; 
    }
    ...
    try {
      ...
      const { data: targetVeh, error: searchError } = await supabase
        .from('vehicles')
        .select('plate')
      ...
      if (!targetVeh) {
        setErrorMsg('Veículo não encontrado na base');
        return;
      }
      ...
      // 5. Cleanup silencioso (fora do fluxo principal para não travar)
  ...
  const cleanupOldAlerts = async (plate: string) => {
    try {
      ...
      console.warn("Erro no cleanup silencioso:", e);
    }
  };
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50 relative overflow-hidden pt-safe" onClick={initAudio}>
      ...
      <div className="bg-blue-600 pb-12 pt-6 px-5 rounded-b-[2.5rem] shadow-lg relative shrink-0">
        <div className="flex justify-between items-center relative z-10">
          <div>
            ...
            <h1 className="text-white text-xl font-black truncate max-w-[200px]">Olá, {user?.fullName?.split(' ')[0]}</h1>
          </div>
      ...
      <div className="flex-grow overflow-y-auto no-scrollbar px-5 pt-4 pb-32">
        <div className="grid grid-cols-1 gap-2 mb-4">
          {PRECONFIGURED_ALERTS.map((alert) => (
            <button 
              ...
              className="flex items-center p-4 bg-white border border-gray-100 rounded-2xl active:bg-blue-50 transition-all text-left shadow-sm group disabled:opacity-50"
            >
              <div className="p-2.5 rounded-xl mr-4 bg-gray-50">
                {renderIcon(alert.icon, "w-5 h-5")}
              </div>
        ...
        <textarea 
          className="w-full p-4 bg-white border border-gray-100 rounded-3xl h-20 focus:ring-2 focus:ring-blue-500 outline-none font-bold text-xs resize-none shadow-sm mb-3 disabled:opacity-50" 
        ...
        <button 
          ...
          className="w-full py-4.5 bg-blue-600 text-white font-black rounded-2xl flex items-center justify-center space-x-3 shadow-xl uppercase text-xs active:scale-[0.97] transition-all disabled:bg-blue-400"
        >
          {sending ? <Loader2 className="animate-spin w-4 h-4" /> : <Send size={18} />} 
          <span>{sending ? 'Enviando...' : 'Avisar Agora'}</span>
        </button>
      </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-gray-100 px-8 pt-4 pb-[calc(1rem+var(--sab))] z-[100] flex justify-around items-center">
        ...
        <button onClick={() => { stopAlarm(); setShowParkingModal(true); setShowHistory(false); }} className={`flex flex-col items-center space-y-1 ${showParkingModal ? 'text-blue-600' : 'text-gray-300'}`}>
          ...
          <div className="bg-white w-full rounded-t-[3rem] p-6 pb-[calc(2.5rem+var(--sab))] animate-in slide-in-from-bottom duration-300" onClick={(e) => e.stopPropagation()}>
             <div className="flex justify-between items-center mb-6">
                <div>
                  ...
                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Localização Atualizada</p>
                </div>
             ...
             <div className="grid grid-cols-2 gap-4 mb-6">
                <div className="bg-blue-50 rounded-[2.5rem] p-4 flex flex-col items-center justify-center border-2 border-blue-100 shadow-inner overflow-hidden">
                   <div className="relative w-24 h-24 mb-3 flex items-center justify-center">
                      <div className="absolute inset-0 border-2 border-dashed border-blue-200 rounded-full animate-spin-slow"></div>
                      <div style={{ transform: `rotate(${arrowRotation}deg)` }} className="transition-transform duration-300 z-10">
                        <Navigation size={42} className="text-blue-600 fill-blue-600 drop-shadow-lg" />
                      </div>
                      <div className="absolute top-0 text-[8px] font-black text-blue-300">CARRO</div>
                   </div>
                   ...
                   <span className="text-[8px] font-black text-blue-400 uppercase tracking-widest">Distância Real</span>
                </div>
                ...
                <div className="bg-gray-50 rounded-[2.5rem] p-2 border-2 border-gray-100 flex items-center justify-center relative overflow-hidden shadow-inner aspect-square">
                   {parkedLocation?.photo ? (
                      ...
                      <div className="flex flex-col items-center text-gray-300">
                        <Camera size={32} />
                        <span className="text-[9px] font-bold mt-2 uppercase">Sem Foto</span>
                      </div>
                   )}
                   <button 
                    onClick={() => fileInputRef.current?.click()}
                    className="absolute bottom-2 right-2 bg-white p-3 rounded-2xl shadow-lg border border-gray-100 active:scale-90 transition-transform"
                   >
             ...
             <div className="space-y-3">
               <button onClick={handleMarkLocation} className="w-full py-5 bg-blue-600 text-white font-black rounded-3xl uppercase shadow-xl flex items-center justify-center space-x-3 active:scale-95 transition-all">
                  <MapPin size={20} /> <span>Marcar Localização Agora</span>
               </button>
               {parkedLocation && (
                  <div className="text-center mb-1">
                    <p className="text-[9px] font-black text-gray-400 uppercase">Última marcação: {new Date(parkedLocation.timestamp).toLocaleTimeString()}</p>
                  </div>
               )}
               <div className="grid grid-cols-2 gap-3">
                 <button onClick={handleOpenMaps} className="py-4 bg-gray-50 text-gray-700 font-bold rounded-2xl uppercase text-[10px] flex items-center justify-center space-x-2 border border-gray-100 active:bg-gray-100">
                    <MapIcon size={16} className="text-blue-500" /> <span>Abrir no Maps</span>
                 </button>
                 <button 
                  onClick={handleShareLocation} 
                  disabled={sharing}
                  className={`py-4 bg-blue-50 text-blue-700 font-bold rounded-2xl uppercase text-[10px] flex items-center justify-center space-x-2 border border-blue-100 active:bg-blue-100 transition-opacity ${sharing ? 'opacity-50 cursor-not-allowed' : ''}`}
                 >
                    {sharing ? <Loader2 size={16} className="animate-spin" /> : <Share2 size={16} className="text-blue-600" />}
                    <span>{sharing ? 'Carregando...' : 'Compartilhar'}</span>
                 </button>
               </div>
             </div>
          </div>
      ...
      {showHistory && (
        <div className="absolute inset-0 bg-black/60 backdrop-blur-sm z-[200] flex items-end" onClick={() => setShowHistory(false)}>
          <div className="bg-white w-full rounded-t-[2.5rem] p-6 pb-[calc(2rem+var(--sab))] animate-in slide-in-from-bottom duration-300" onClick={(e) => e.stopPropagation()}>
             <div className="flex justify-between items-center mb-6">
                <h3 className="text-lg font-black uppercase italic text-gray-800">Alertas Recentes (Top 2)</h3>
                <button onClick={() => setShowHistory(false)} className="text-gray-300"><XCircle size={24} /></button>
             </div>
             <div className="space-y-2.5 pb-4">
               {alertHistory.length === 0 ? (
                 <p className="text-center py-8 text-gray-400 font-bold uppercase text-[10px]">Sem alertas</p>
               ) : (
                 alertHistory.map(a => (
                   <div key={a.id} className="p-3.5 bg-gray-50 rounded-2xl border border-gray-100 flex items-center space-x-3">
                      <div className="bg-white p-2 rounded-xl shadow-sm">{renderIcon(a.icon, "w-4 h-4")}</div>
                      <div className="flex-grow">
                        <p className="text-[9px] font-black text-gray-400 uppercase">DE: {a.sender_name.split(' ')[0]}</p>
                        <p className="text-xs font-bold text-gray-700">"{a.message}"</p>
                      </div>
                      <span className="text-[8px] font-black text-gray-300">{new Date(a.created_at).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}</span>
                   </div>
                 ))
               )}
             </div>
      ...
      {/* MODAL DE ALERTA RECEBIDO */}
      {showReceivedModal && activeAlert && (
        <div className="absolute inset-0 bg-gray-900/40 backdrop-blur-md z-[500] flex items-center justify-center p-6 animate-in fade-in duration-300">
           <div className="bg-white w-full max-w-xs rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in duration-300">
              <div className="bg-blue-600 p-6 flex flex-col items-center">
                 <div className="bg-white p-4 rounded-full mb-3 animate-bounce shadow-inner">
                    {renderIcon(activeAlert.icon, "w-10 h-10")}
                 </div>
                 <h2 className="text-white text-xl font-black uppercase italic">Alerta Novo!</h2>
              </div>
              <div className="p-6 space-y-5 text-center">
                 <div className="inline-block bg-blue-50 text-blue-600 px-4 py-1 rounded-full text-xs font-black uppercase tracking-widest">{vehicle?.plate}</div>
                 <div className="bg-gray-50 p-6 rounded-3xl border border-gray-100">
                    <p className="text-gray-800 text-lg font-black italic">"{activeAlert.message}"</p>
                 </div>
                 <p className="text-[10px] font-black text-gray-400 uppercase">DE: {activeAlert.sender_name.split(' ')[0]}</p>
                 <button 
                    onClick={() => { 
                      stopAlarm();
                      playConfirmationBeeps();
                    ...
                    className="w-full py-4.5 bg-blue-600 text-white font-black rounded-2xl text-sm uppercase shadow-lg shadow-blue-100 active:scale-95 transition-all"
                 >ENTENDIDO</button>
              </div>
           </div>
        </div>
