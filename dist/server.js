...
const findUserByPlate = async (plate) => {
    if (useMockDatabase || !pool) {
...
app.post('/auth/register', async (req, res) => {
    try {
        ...
        const result = await pool.query('INSERT INTO users (name, email, phone, password) VALUES ($1, $2, $3, $4) RETURNING id, name, email, phone', [name, email, phone, password] // In a real app, this should be hashed
        );
...
app.post('/vehicles', async (req, res) => {
    try {
        ...
        const result = await pool.query('INSERT INTO vehicles (plate, model, color, state, user_id) VALUES ($1, $2, $3, $4, $5) RETURNING *', [plate, model, color, state, userId]);
...
app.post('/alerts/send', async (req, res) => {
    try {
        ...
        if (useMockDatabase || !pool) {
            ...
            const result = await pool.query('INSERT INTO alerts (target_plate, message, sender_id) VALUES ($1, $2, $3) RETURNING *', [targetPlate, message, userId]);
            newAlert = {
                id: result.rows[0].id,
